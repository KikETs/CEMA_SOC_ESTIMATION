...
ROOT=Path('/home/lab/바탕화면/DL/CEMA_LFP/lfp_confirmatory_minipanel'); OUT=ROOT/'tier3_inference_only'; OUT.mkdir(exist_ok=True)
...
for p in profiles:
 raw_by_temp={t:pd.read_csv(ROOT/f'prepared_data_ocv_discharge_soc/{t}C/LFP_{t}C_{p}.csv') for t in temps}
 for f in features:
  for s in (0,1,2):
...
for t in temps:
 for p in profiles:
  path=ROOT/f'prepared_data_ocv_discharge_soc/{t}C/LFP_{t}C_{p}.csv'; d=pd.read_csv(path)

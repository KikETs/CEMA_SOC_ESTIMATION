...
class Candidate:
    ...
    hard_soc: float = 0.2
    hard_dynamic: float = 0.0
    dropout: float = 0.06


CANDIDATES = (
    Candidate("gru_normal_g4_coldlow20_w3", hard_weight=3.0, hard_soc=0.20),
    Candidate("gru_normal_g4_coldlow20_w6", hard_weight=6.0, hard_soc=0.20),
    Candidate("gru_normal_g4_coldlow25_w4", hard_weight=4.0, hard_soc=0.25),
    Candidate("gru_normal_g4_temp_hard_affine", temp_mode="hard_affine"),
    Candidate("gru_normal_g4_temp_hard_heads", temp_mode="hard_heads"),
    Candidate("gru_normal_g4_temp_heads_coldlow20_w3", temp_mode="hard_heads", hard_weight=3.0, hard_soc=0.20),
)
...
def prefix_for(candidate: Candidate, seeds: tuple[int, ...], epochs: int, batch_size: int) -> str:
    ...
    return f"soc80_us06_lowsoc_temphead_screen_{candidate.name}_{seed_tag}_b{int(batch_size)}_e{int(epochs)}"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Screen low-SOC loss and temperature-head GRU G4 candidates on US06.")
...
def main() -> None:
    ...
    pd.DataFrame(manifest_rows).to_csv(out_dir / "soc80_us06_lowsoc_temphead_screen_manifest.csv", index=False)

    print(
        f"us06_lowsoc_temphead_screen jobs={len(CANDIDATES)} seeds={args.seeds} "
    ...
    print(f"us06_lowsoc_temphead_screen finished completed={completed} skipped={skipped}", flush=True)

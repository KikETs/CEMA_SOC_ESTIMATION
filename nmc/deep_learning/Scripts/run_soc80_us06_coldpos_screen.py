...
CANDIDATES = (
    Candidate("gru_residual_eqdyn_coldpos_l15_w2_hard3", "anchor_residual_cold_low_soc_positive_sequence", "gru", "paper_g4_eqdyn", 2.0, 0.15, 3.0),
    Candidate("gru_residual_eqdyn_coldpos_l25_w2_hard3", "anchor_residual_cold_low_soc_positive_sequence", "gru", "paper_g4_eqdyn", 2.0, 0.25, 3.0),
    Candidate("gru_normal_eqdyn_coldpos_l15_w4_hard3", "normal_cold_low_soc_positive_sequence", "gru", "paper_g4_eqdyn", 4.0, 0.15, 3.0),
    Candidate("gru_normal_eqdyn_coldpos_l25_w4_hard3", "normal_cold_low_soc_positive_sequence", "gru", "paper_g4_eqdyn", 4.0, 0.25, 3.0),
)
...
def prefix_for(candidate: Candidate, seeds: tuple[int, ...], epochs: int, batch_size: int) -> str:
    ...
    return f"soc80_us06_coldpos_screen_{candidate.name}_{seed_tag}_b{int(batch_size)}_e{int(epochs)}"
...
def cfg_for_candidate(candidate: Candidate, args: argparse.Namespace, prefix: str) -> TrainDSTSelectorConfig:
    ...
    return TrainDSTSelectorConfig(
        ...
        hard_region_soc_threshold=0.4,
        hard_region_dynamic_weight=0.5,
        hard_region_cold_only=True,
        cold_corrector_limit=float(candidate.correction_limit),
        cold_corrector_soc_threshold=0.4,
        cold_corrector_temp_threshold=-0.45,
        cold_corrector_gate_sharpness=8.0,
        cold_corrector_sag_gate=bool(candidate.sag_gate),
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Screen one-sided cold low-SOC correctors on US06.")
...
def main() -> None:
    ...
    pd.DataFrame(manifest_rows).to_csv(out_dir / "soc80_us06_coldpos_screen_manifest.csv", index=False)

    print(
        f"us06_coldpos_screen jobs={len(CANDIDATES)} seeds={args.seeds} epochs={args.epochs} batch_size={args.batch_size}",
        flush=True,
    )
    ...
    print(f"us06_coldpos_screen finished completed={completed} skipped={skipped}", flush=True)

...
def prefix_for(holdout: str, seeds: tuple[int, ...], epochs: int, batch_size: int) -> str:
    ...
    return (
        ...
        f"paper_g4_eqdyn_holdout{holdout.lower()}_"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run GRU residual G4 equilibrium/dynamic voltage LOPO experiment.")
...
def main() -> None:
    ...
    pd.DataFrame(manifest_rows).to_csv(out_dir / "soc80_g4_eqdyn_gru_residual_manifest.csv", index=False)

    print(
        f"g4_eqdyn_gru_residual jobs={len(holdouts)} seeds={args.seeds} "
    ...
    print(f"g4_eqdyn_gru_residual finished completed={completed} skipped={skipped}", flush=True)

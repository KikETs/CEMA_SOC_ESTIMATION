...
FEATURE_SETS = (
    ...
    "paper_g4_eqdyn_no_vcorr_cold_relax",
    "paper_g4_eqdyn_no_vcorr_cold_current",
...
def feature_tag(feature_set: str) -> str:
    return str(feature_set).removeprefix("paper_g4_eqdyn_no_vcorr").strip("_") or "base"


def build_jobs() -> list[Job]:
    return [Job(feature_set=feature_set, holdout=holdout) for feature_set in FEATURE_SETS for holdout in HOLDOUTS]


def prefix_for(job: Job) -> str:
    if job.feature_set == "paper_g4_eqdyn_no_vcorr":
        return (
    ...
    return (
        "endzero_lopo4_rtvar_lowv_ema120_gru_residual_eqdyn_no_vcorr_"
        f"{feature_tag(job.feature_set)}_holdout{job.holdout.lower()}_seed0_b2048_e200"
    )
...
def write_ranked_outputs(out_dir: Path, manifest_rows: list[dict]) -> None:
    manifest = pd.DataFrame(manifest_rows)
    manifest_path = out_dir / "endzero_rtvar_lowv_ema120_cold_feature_gru_residual_lopo_manifest.csv"
    manifest.to_csv(manifest_path, index=False)
    done = manifest[manifest["status"].isin(["completed", "skipped_existing"])].copy()
    if done.empty:
        return
    done.sort_values(["mean_mae", "max_mae"]).to_csv(
        out_dir / "endzero_rtvar_lowv_ema120_cold_feature_gru_residual_lopo_ranked.csv",
        index=False,
    )
    done.sort_values(["holdout", "mean_mae", "max_mae"]).to_csv(
        out_dir / "endzero_rtvar_lowv_ema120_cold_feature_gru_residual_lopo_holdout_ranked.csv",
        index=False,
    )
    (
        done.groupby("feature_set", as_index=False)
        .agg(
            n=("holdout", "count"),
        ...
        .sort_values(["mean_mae", "max_mae"])
        .to_csv(out_dir / "endzero_rtvar_lowv_ema120_cold_feature_gru_residual_lopo_feature_agg.csv", index=False)
    )


def main() -> None:
    out_dir = ROOT / "nmc_goal_vcorr_it_train_dst_selector_results"
    log_dir = out_dir / "logs" / "cold_feature_gru_residual"
    ...
    print(
        f"start cold_feature_gru_residual jobs={len(jobs)} feature_sets={','.join(FEATURE_SETS)} "
        f"holdouts={','.join(HOLDOUTS)}",
        flush=True,
    )
    for idx, job in enumerate(jobs):
        ...
        row = {
            "idx": idx,
            "feature_set": job.feature_set,
            "feature_tag": feature_tag(job.feature_set),
            "model": "gru",
            "head": "residual",
            "model_kind": "anchor_residual_sequence",
            "recurrent": "gru",
        ...
        if summary_path.exists():
            ...
            print(f"skip {idx + 1}/{len(jobs)} {feature_tag(job.feature_set)}/{job.holdout}", flush=True)
            write_ranked_outputs(out_dir, manifest_rows)
            continue
        t0 = time.time()
        print(f"run {idx + 1}/{len(jobs)} {feature_tag(job.feature_set)}/{job.holdout}", flush=True)
        try:
            with log_path.open("w") as log_fh, redirect_stdout(log_fh), redirect_stderr(log_fh):
                print(
                    f"job idx={idx} feature_set={job.feature_set} holdout={job.holdout} "
                    f"train_profiles={','.join(train_profiles_for_holdout(job.holdout))}",
                    flush=True,
                )
            ...
            print(
                f"done {idx + 1}/{len(jobs)} {feature_tag(job.feature_set)}/{job.holdout} "
            ...
            print(f"failed {idx + 1}/{len(jobs)} {feature_tag(job.feature_set)}/{job.holdout}: {type(exc).__name__}: {exc}", flush=True)
        manifest_rows.append(row)
        write_ranked_outputs(out_dir, manifest_rows)
        gc.collect()
    write_ranked_outputs(out_dir, manifest_rows)

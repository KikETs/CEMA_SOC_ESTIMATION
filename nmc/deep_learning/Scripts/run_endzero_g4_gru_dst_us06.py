...
def cfg_for_holdout(holdout: str) -> TrainDSTSelectorConfig:
    prefix = f"endzero_lopo2_baseline_model_gru_residual_head_paper_g4_all_ema_holdout{holdout.lower()}_seed0_b2048_e200"
...
def main() -> None:
    ...
    for holdout in HOLDOUTS:
        ...
        t0 = time.time()
        print(
            f"=== start label=endzero model=GRU_G4_residual holdout={holdout} "
            f"train={','.join(cfg.train_profiles)} seed=0 epochs=200 ===",
            flush=True,
        )
        run(cfg)
        elapsed = time.time() - t0
        summary_path = out_dir / f"{cfg.output_prefix}_test_summary.csv"
        by_temp_path = out_dir / f"{cfg.output_prefix}_by_temperature.csv"
        rows.append(
            {
                "holdout": holdout,
    ...
    manifest_path = out_dir / "endzero_g4_gru_dst_us06_manifest.csv"

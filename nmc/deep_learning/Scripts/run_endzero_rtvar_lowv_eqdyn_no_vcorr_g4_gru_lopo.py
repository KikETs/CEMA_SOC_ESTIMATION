...
def cfg_for_holdout(holdout: str) -> TrainDSTSelectorConfig:
    prefix = f"endzero_lopo4_rtvar_lowv_ema120_gru_residual_eqdyn_no_vcorr_holdout{holdout.lower()}_seed0_b2048_e200"
...
def main() -> None:
    ...
    for holdout in HOLDOUTS:
        ...
        t0 = time.time()
        print(
            f"=== start label=endzero v_corr=rtvar_lowv_ema120 model=GRU_residual "
            f"feature={FEATURE_SET} holdout={holdout} train={','.join(cfg.train_profiles)} "
            f"seed=0 epochs=200 ===",
            flush=True,
        )
        run(cfg)
        ...
        rows.append(
            {
                ...
                "rtvar_params_path": str(out_dir / f"{cfg.output_prefix}_rtvar_lowv_params.csv"),
                "elapsed_s": elapsed,
            }
        )
        print(f"=== done holdout={holdout} elapsed_s={elapsed:.1f} ===", flush=True)
    manifest = pd.DataFrame(rows)
    manifest_path = out_dir / "endzero_rtvar_lowv_ema120_eqdyn_no_vcorr_g4_gru_lopo_manifest.csv"

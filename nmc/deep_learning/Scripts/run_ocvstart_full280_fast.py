...
def prefix_for(job: Job, seeds: tuple[int, ...], epochs: int, batch_size: int, prefix_tag: str = "ocvstart") -> str:
    seed_tag = "seeds" + "".join(str(seed) for seed in seeds)
    batch_tag = f"b{int(batch_size)}_opt123preidx"
    lower_holdout = job.holdout.lower()
    tag = sanitize(prefix_tag)
    if job.suite in {"feature_ablation", "baseline_model"}:
        return (
            f"{tag}_lopo4_{job.suite}_{sanitize(job.exp)}_{job.head}_head_"
            f"{job.feature_set}_holdout{lower_holdout}_{seed_tag}_{batch_tag}_e{epochs}"
        )
    return (
        f"{tag}_lopo4_feature_ablation_{sanitize(job.exp + '_' + job.feature_set)}_{job.head}_head_"
        f"{job.feature_set}_holdout{lower_holdout}_{seed_tag}_{batch_tag}_e{epochs}"
    )
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run OCV-start full 280-job CEMA/MLP grid in one Python process.")
    p.add_argument("--base-dir", default=str(Path(__file__).resolve().parents[1]))
    p.add_argument("--raw-root", default="nmc_ocvstart_lopo_clean")
    p.add_argument("--seeds", default="0,1,2")
    p.add_argument("--epochs", type=int, default=200)
    p.add_argument("--batch-size", type=int, default=256)
    p.add_argument("--start-index", type=int, default=0)
    p.add_argument("--limit", type=int, default=0)
    p.add_argument("--prefix-tag", default="ocvstart")
...
def main() -> None:
    ...
    manifest_name = f"{sanitize(args.prefix_tag)}_full280_fast_job_manifest.csv"
    pd.DataFrame(manifest_rows).to_csv(out_dir / manifest_name, index=False)

    print(
        f"ocvstart_fast_full280 total_jobs={len(jobs)} selected_jobs={len(selected_jobs)} seeds={args.seeds} "
        f"epochs={args.epochs} batch_size={args.batch_size} prefix_tag={args.prefix_tag} "
        f"include_train_final_eval={bool(args.include_train_final_eval)}",
        flush=True,
    )
    started = time.time()

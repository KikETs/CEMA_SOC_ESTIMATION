...
SETTINGS = ("base", "profile_cvar0c", "temp_L")
...
def cfg_for_job(job: Job, prefix: str) -> TrainDSTSelectorConfig:
    model_kind = "anchor_residual_temp_limit_sequence" if job.setting == "temp_L" else "anchor_residual_sequence"
    lambda_profile_cvar = 2.0 if job.setting == "profile_cvar0c" else 0.0
    return TrainDSTSelectorConfig(
        ...
        lambda_cold_profile_socbin_cvar=lambda_profile_cvar,
        cold_profile_socbin_cvar_top_frac=0.25,
        cold_profile_socbin_cvar_bin_width=0.1,
        cold_profile_socbin_cvar_temp=0.0,
        lambda_condinv=0.02,
        lambda_rex=2.0,
        weight_0=0.8,
        weight_25=2.2,
...
def write_outputs(out_dir: Path, rows: list[dict]) -> None:
    ...
    manifest_path = out_dir / "endzero_rtvar_lowv_ema120_gru_residual_0c_robust_tempL_lopo_manifest.csv"
    ...
    comp.sort_values(["holdout", "mae_0C", "mean_mae"]).to_csv(
        out_dir / "endzero_rtvar_lowv_ema120_gru_residual_0c_robust_tempL_lopo_delta_vs_base.csv",
        index=False,
    )
    (
        ...
        .sort_values(["mae_0C", "mean_mae"])
        .to_csv(out_dir / "endzero_rtvar_lowv_ema120_gru_residual_0c_robust_tempL_lopo_setting_agg.csv", index=False)
    )


def main() -> None:
    out_dir = ROOT / "nmc_goal_vcorr_it_train_dst_selector_results"
    log_dir = out_dir / "logs" / "gru_residual_0c_robust_tempL"
    ...
    print(
        f"start gru_residual_0c_robust_tempL jobs={len(jobs)} settings={','.join(SETTINGS)} "

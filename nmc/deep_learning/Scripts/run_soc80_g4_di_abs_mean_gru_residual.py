...
def prefix_for(holdout: str, seeds: tuple[int, ...], epochs: int, batch_size: int) -> str:
    ...
    return (
        "soc80_nofloor_lopo4_baseline_model_gru_residual_head_"
        f"paper_g4_all_ema_di_abs_mean_holdout{holdout.lower()}_"
        f"{seed_tag}_b{int(batch_size)}_opt123preidx_e{int(epochs)}"
    )
...
def cfg_for_holdout(holdout: str, args: argparse.Namespace, prefix: str) -> TrainDSTSelectorConfig:
    ...
    return TrainDSTSelectorConfig(
        ...
        head_kind="linear",
        dropout=0.06,
        feature_set="paper_g4_all_ema_di_abs_mean",
        stage2_feature_set="paper_g4_all_ema_di_abs_mean",
        temp_mode="none",
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run focused GRU residual G4+dI_abs_mean SOC80 LOPO experiment.")
...
def main() -> None:
    ...
    pd.DataFrame(rows).to_csv(out_dir / "soc80_g4_di_abs_mean_gru_residual_manifest.csv", index=False)

    print(
        f"g4_di_abs_mean_gru_residual jobs={len(holdouts)} seeds={args.seeds} "
    ...
    print(f"g4_di_abs_mean_gru_residual finished completed={completed} skipped={skipped}", flush=True)

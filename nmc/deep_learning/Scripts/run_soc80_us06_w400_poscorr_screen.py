...
CANDIDATES = (
    Candidate("w400_poscorr_lim002_thr025", 0.02, 0.25, False),
    Candidate("w400_poscorr_lim004_thr025", 0.04, 0.25, False),
    Candidate("w400_poscorr_lim004_thr035", 0.04, 0.35, False),
    Candidate("w400_poscorr_lim004_thr025_sag", 0.04, 0.25, True),
)
...
def prefix_for(candidate: Candidate, seeds: tuple[int, ...], epochs: int, batch_size: int) -> str:
    ...
    return f"soc80_us06_w400_poscorr_screen_{candidate.name}_{seed_tag}_b{int(batch_size)}_e{int(epochs)}"
...
def cfg_for_candidate(candidate: Candidate, args: argparse.Namespace, prefix: str) -> TrainDSTSelectorConfig:
    ...
    return TrainDSTSelectorConfig(
        ...
        layers=1,
        kernel_size=5,
        model_kind="normal_cold_low_soc_positive_sequence",
        recurrent="gru",
        head_kind="linear",
        ...
        cold_corrector_soc_threshold=float(candidate.soc_threshold),
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Screen w400 GRU positive cold low-SOC correction branch on US06.")
...
def main() -> None:
    ...
    pd.DataFrame(manifest_rows).to_csv(out_dir / "soc80_us06_w400_poscorr_screen_manifest.csv", index=False)

    print(
        f"us06_w400_poscorr_screen jobs={len(candidates)} seeds={args.seeds} "
    ...
    summary_path = out_dir / "soc80_us06_w400_poscorr_screen_summary.csv"
    summary.to_csv(summary_path, index=False)
    print(f"us06_w400_poscorr_screen finished completed={completed} skipped={skipped} summary={summary_path}", flush=True)
    if len(summary):

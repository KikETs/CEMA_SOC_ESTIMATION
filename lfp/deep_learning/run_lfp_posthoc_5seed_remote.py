 ...
 for condition in panel.CONDITIONS:
  for feature in FEATURES:
   for holdout in panel.PROFILES:
    ...
    (OUT/'job_manifest.json').write_text(json.dumps({'analysis_stage':'posthoc_5seed','jobs':jobs},indent=2)+'\n')
 after={str(p):sha(p) for p in protected}
 if before!=after: raise RuntimeError('preregistered artifact hash changed')
 (OUT/'preregistration_guard.json').write_text(json.dumps({'analysis_stage':'posthoc_5seed','protected_hashes_before':before,'protected_hashes_after':after,'unchanged':True,'new_training_only':{'features':list(FEATURES),'seeds':list(SEEDS),'conditions':[c.name for c in panel.CONDITIONS],'holdouts':list(panel.PROFILES),'individual_runs':36}},indent=2)+'\n')

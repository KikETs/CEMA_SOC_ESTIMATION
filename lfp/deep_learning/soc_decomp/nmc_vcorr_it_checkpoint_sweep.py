...
from .nmc_vcorr_it_goal_remote_screen import (
    GoalScreenConfig,
    Variant,
    VcorrITGoalModel,
    group_keys,
...
BASE_PREFIX = "nmc_goal_vcorr_it_h64_w50_remote_checkpoint_sweep_seed0"
...
def sweep_variants(variant_set: str = "base") -> list[Variant]:
    if variant_set == "tri45":
        return [
            Variant(
                "tri45_tempmix_mlp_lstm2_rex2_w25x3_w45x2",
                recurrent="lstm",
                layers=2,
                head_kind="mlp",
                temp_mode="moe",
            ...
            Variant(
                "tri45_tempmix_mlp_lstm2_rex2_w25x4_w45x2",
                recurrent="lstm",
                layers=2,
                head_kind="mlp",
                temp_mode="moe",
            ...
            Variant(
                "tri45_tempmix_mlp_lstm2_rex2_w25x3_w45x3",
                recurrent="lstm",
                layers=2,
                head_kind="mlp",
                temp_mode="moe",
            ...
            Variant(
                "tri45_tempbias_mlp_lstm2_rex2_w25x3_w45x2",
                recurrent="lstm",
                layers=2,
                head_kind="mlp",
                temp_mode="bias",
            ...
            Variant(
                "tri45_tempmix_linear_lstm2_rex2_w25x3_w45x2",
                recurrent="lstm",
                layers=2,
                head_kind="linear",
                temp_mode="moe",
    ...
    if variant_set == "target25":
        return [
            Variant(
                "sweep_tempmix_mlp_lstm2_rex2_w25x2",
                recurrent="lstm",
                layers=2,
                head_kind="mlp",
                temp_mode="moe",
            ...
            Variant(
                "sweep_tempmix_mlp_lstm2_rex2_w25x3",
                recurrent="lstm",
                layers=2,
                head_kind="mlp",
                temp_mode="moe",
            ...
            Variant(
                "sweep_tempmix_mlp_lstm2_rex2_w25x4",
                recurrent="lstm",
                layers=2,
                head_kind="mlp",
                temp_mode="moe",
            ...
            Variant(
                "sweep_tempmix_mlp_lstm2_rex0p5_w25x3",
                recurrent="lstm",
                layers=2,
                head_kind="mlp",
                temp_mode="moe",
            ...
            Variant(
                "sweep_tempbias_mlp_lstm2_rex2_w25x3",
                recurrent="lstm",
                layers=2,
                head_kind="mlp",
                temp_mode="bias",
    ...
    return [
        Variant("sweep_tempmix_linear_lstm2_rex2", recurrent="lstm", layers=2, head_kind="linear", temp_mode="moe", lambda_rex=2.0),
        Variant("sweep_tempmix_mlp_lstm2_rex2", recurrent="lstm", layers=2, head_kind="mlp", temp_mode="moe", lambda_rex=2.0),
        Variant("sweep_tempbias_mlp_lstm2_rex2", recurrent="lstm", layers=2, head_kind="mlp", temp_mode="bias", lambda_rex=2.0),
        Variant("sweep_tempmix_linear_tcn5_w0x2_w25x2", recurrent="tcn", layers=5, head_kind="linear", temp_mode="moe", dropout=0.04, lambda_rex=2.0, weight_0=2.0, weight_25=2.0),
    ]
...
def run_variant(cfg: SweepConfig, variant: Variant, frames, out_dir: Path) -> pd.DataFrame:
    ...
    for ep in range(1, int(cfg.epochs) + 1):
        ...
        if ep == 1 or ep % int(cfg.eval_every) == 0 or ep == int(cfg.epochs):
            ...
            test_p = test.pivot_table(index=["variant", "epoch", "split"], columns="temperature_C", values="MAE_pct").reset_index()
            if len(test_p):
                mae0 = float(test_p.get(0.0, pd.Series([np.nan])).iloc[0])
                mae25 = float(test_p.get(25.0, pd.Series([np.nan])).iloc[0])
                mae45 = float(test_p.get(45.0, pd.Series([np.nan])).iloc[0])
                print(f"{variant.name} epoch={ep} train_loss={np.mean(losses):.5f} test0={mae0:.3f}% test25={mae25:.3f}% test45={mae45:.3f}%", flush=True)
        elif ep % int(cfg.print_every) == 0:
            print(f"{variant.name} epoch={ep} train_loss={np.mean(losses):.5f}", flush=True)
    return pd.concat(rows, ignore_index=True) if rows else pd.DataFrame()


def run(cfg: SweepConfig) -> pd.DataFrame:
    ...
    if int(cfg.hidden_size) != 64 or int(cfg.window_len) != 50:
        raise ValueError("This sweep is fixed to hidden_size=64 and window_len=50.")
    out_dir = cfg.base_dir / "nmc_goal_vcorr_it_checkpoint_sweep_results"
    ...
    all_rows = []
    for variant in sweep_variants(cfg.variant_set):
        set_seed(cfg.seed)
        print(f"===== sweep {variant.name} =====", flush=True)
    ...
    for (variant, epoch, split), idx in out.groupby(["variant", "epoch", "split"]).groups.items():
        sub = out.loc[idx]
        ...
        ok45 = ok and len(mae45) and float(mae45.iloc[0]) < 0.3
        out.loc[idx, "goal_0C_lt1_25C_lt0p7_45C_lt0p3"] = bool(ok45 and split == "test")
    out.to_csv(out_dir / f"{cfg.output_prefix}_by_temperature.csv", index=False)
    metadata = {**asdict(cfg), "feature_columns": FEATURE_COLS, "input_feature_dim": len(FEATURE_COLS), "uses_soc_input": False, "uses_cumulative_input": False, "uses_explicit_current_integration": False, "uses_amp": False}
    (out_dir / f"{cfg.output_prefix}_metadata.json").write_text(json.dumps(metadata, indent=2, default=str), encoding="utf-8")
    print("Sweep best test rows:")
    ...
    if len(piv):
        ...
        piv["max_0_25_45"] = piv[[0.0, 25.0, 45.0]].max(axis=1)
        print(piv.sort_values(["max_0_25", 25.0]).head(20).to_string(index=False), flush=True)
        goal = piv[(piv[0.0] < 1.0) & (piv[25.0] < 0.7) & (piv[45.0] < 0.3)]
        if len(goal):
            print("TRI45_GOAL_ROWS", flush=True)
            print(goal.sort_values(["max_0_25_45", 25.0]).to_string(index=False), flush=True)
    return out


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Remote checkpoint sweep for fixed Vcorr/I/T models.")
    p.add_argument("--base-dir", default=".")
    p.add_argument("--raw-root", default=SweepConfig.raw_root)
    ...
    p.add_argument("--variant-set", default="base", choices=["base", "target25", "tri45"])
    return p.parse_args()

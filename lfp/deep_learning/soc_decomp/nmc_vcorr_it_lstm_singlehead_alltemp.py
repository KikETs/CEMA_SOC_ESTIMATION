...
BASE_PREFIX = "nmc_vcorr_it_lstm_singlehead_l1_h64_w50_alltemp_seed0"


@dataclass
class NMCVcorrITSingleHeadAllTempConfig:
    base_dir: Path = Path(".")
...
def train_all_temperature(
    cfg: NMCVcorrITSingleHeadAllTempConfig,
    ...
    for df in (pred, valid_pred):
        ...
        df["temperature_train_test_mode"] = "all_temperature"
    ...
    metadata = {
        **asdict(cfg),
        ...
        "temperature_mode": "one model trained on all temperatures",
...
def write_report(
    ...
    lines = [
        "# NMC Vcorr/I/T Single-Head LSTM All-Temperature",
        "",
        "## Fixed condition",
        ...
        "- Temperature mode: one model trained on 0/25/45 C together.",
        "- Model: input projection 3->64, LSTM layer=1 hidden=64, LayerNorm, out_proj Linear(64->1), sigmoid",
        ...
        "",
        "## Test overall",
        table_md(res["overall"], ["model_name", "n_windows", "MAE_pct", "RMSE_pct", "selected_epoch"]),
        "",
        "## Valid overall",
        table_md(res["valid_overall"], ["model_name", "n_windows", "MAE_pct", "RMSE_pct", "selected_epoch"]),
        "",
        "## Test by temperature",
        table_md(res["by_temperature"], ["temperature_C", "n_windows", "MAE_pct", "RMSE_pct", "jitter_ratio"]),
        "",
        "## Focus",
        table_md(res["focus"], ["scope", "n_windows", "MAE_pct", "RMSE_pct", "catastrophic_gt5_pct"]),
        "",
...
def run(cfg: NMCVcorrITSingleHeadAllTempConfig) -> dict[str, pd.DataFrame]:
    ...
    out_dir = cfg.base_dir / "nmc_vcorr_it_lstm_singlehead_alltemp_results"
    ...
    res = train_all_temperature(cfg, frames, out_dir)
    write_report(cfg, out_dir, start_audit, r0_df, schema, leakage, res)
    print(f"Report: {out_dir / (cfg.output_prefix + '_report.md')}", flush=True)
    return res


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="NMC Vcorr/I/T single-head 1-layer LSTM all-temperature experiment.")
    p.add_argument("--base-dir", default=".")
    p.add_argument("--raw-root", default=NMCVcorrITSingleHeadAllTempConfig.raw_root)

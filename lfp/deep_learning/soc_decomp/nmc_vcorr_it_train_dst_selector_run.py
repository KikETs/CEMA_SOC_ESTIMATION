...
def _cached_feature_frames(cfg: TrainDSTSelectorConfig, files: list[Path], r0_df: pd.DataFrame) -> dict[str, list[pd.DataFrame]]:
    key = _feature_frame_cache_key(cfg, files, r0_df)
    if key not in _FEATURE_FRAME_CACHE:
        _FEATURE_FRAME_CACHE[key] = add_vit_engineered_features(build_feature_frames(cfg, files, r0_df))
...
def _scaled_frame_cache_key(frames: dict[str, list[pd.DataFrame]], feature_cols: list[str]) -> tuple:
    ...
    return (frame_sig, tuple(feature_cols))
...
class AnchorResidualMLPModel(nn.Module):
    ...
    def __init__(
        ...
        idx = [feature_cols.index(col) for col in preferred if col in feature_cols]
        if "V_corr_raw" not in feature_cols or "T" not in feature_cols:
            raise RuntimeError("AnchorResidualMLPModel requires V_corr_raw and T features.")
        self.anchor_indices = idx
        anchor_dim = len(idx)
        self.anchor_head = nn.Sequential(
            nn.Linear(anchor_dim, int(hidden_size)),
...
def _prepare_seed_shared_assets(cfg: TrainDSTSelectorConfig, frames, out_dir: Path) -> dict:
    ...
    # Transfer-protocol boundary: fit normalization only on the temperatures
    # admitted to each training stage.  The upstream implementation scaled all
    # train-profile temperatures before applying the temperature filter, which
    # leaks held-out-temperature statistics in the 4-temperature experiment.
    stage1_frames = {
        "train": _filter_frame_list_by_temperatures(
            frames["train"], tuple(cfg.train_temperatures), split_name="stage1_pre_scale:train"
        ),
        "valid": _filter_frame_list_by_temperatures(
            frames["valid"], tuple(cfg.valid_temperatures), split_name="stage1_pre_scale:valid"
        ),
        "test": _filter_frame_list_by_temperatures(
            frames["test"], tuple(cfg.test_temperatures), split_name="stage1_pre_scale:test"
        ),
    }
    scaled, _ = _cached_scaled_frames_for_ablation(stage1_frames, feature_cols)
    ...
    if (
        stage2_feature_cols == feature_cols
        and stage2_train_temperatures == tuple(cfg.train_temperatures)
        and stage2_valid_temperatures == tuple(cfg.valid_temperatures)
        and stage2_test_temperatures == tuple(cfg.test_temperatures)
    ):
        scaled_stage2 = scaled
    else:
        stage2_frames = {
            "train": _filter_frame_list_by_temperatures(
                frames["train"], stage2_train_temperatures, split_name="stage2_pre_scale:train"
            ),
            "valid": _filter_frame_list_by_temperatures(
                frames["valid"], stage2_valid_temperatures, split_name="stage2_pre_scale:valid"
            ),
            "test": _filter_frame_list_by_temperatures(
                frames["test"], stage2_test_temperatures, split_name="stage2_pre_scale:test"
            ),
        }
        scaled_stage2, _ = _cached_scaled_frames_for_ablation(stage2_frames, stage2_feature_cols)

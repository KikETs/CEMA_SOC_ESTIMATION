#!/usr/bin/env python3
"""Run the frozen LFP T6-plain headline configuration."""
...
def assert_recipe(cfg, seeds: tuple[int, ...]) -> None:
    ...
    expected_temperatures = (-10.0, 0.0, 10.0, 20.0, 25.0, 30.0, 40.0, 50.0)
    if tuple(cfg.train_temperatures) != expected_temperatures:
        raise RuntimeError(f"Unexpected LFP temperatures: {cfg.train_temperatures!r}")
...
def main() -> None:
    ...
    for holdout in holdouts:
        cfg = panel.make_cfg(condition, "tier1", holdout, "T6", "gru", "normal", seeds, cfg_args)
        prefix = (
            f"paper_t6_plain_lfp_gru_normal_t6_holdout{holdout.lower()}_"
            f"s0123456789_b{args.batch_size}_e{args.epochs}"
        )
        cfg = replace(
            cfg, base_dir=args.output_root.resolve(), raw_root=args.data_root.resolve(), output_prefix=prefix,
            **{name: value for name, value in LOCKED_RECIPE.items() if hasattr(cfg, name)},
        )
    ...
    pd.DataFrame([{
        "chemistry": "LFP", "feature_set": FEATURE_SET, "model": "GRU",

...
class _DynamicGRU(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        return torch.sigmoid(self.base_head(self.encode_sequence(x)))[:, -1, :]


class _PlainLinearGRU(_DynamicGRU):
...
class FrozenPackage:
    def __init__(self, path):
        ...
        if self.config.get("head") != "plain_linear":
            raise RuntimeError(f"Expected plain_linear head, got {self.config.get('head')!r}")
        self.model = _PlainLinearGRU(self.config)
        ...
        with torch.inference_mode():
            for start, stop in segments:
                ...
                for batch_start in range(0, len(windows), batch_size):
                    batch = windows[batch_start : batch_start + batch_size].to(self.device)

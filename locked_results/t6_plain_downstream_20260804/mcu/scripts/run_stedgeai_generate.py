...
def main() -> None:
    ...
    if len(parity) != 18 or set(parity["status"]) != {"PASS"}:
        raise SystemExit("ONNX parity is not a complete 18/18 PASS")
    manifest = pd.read_csv(DROP / "onnx_export_manifest.csv")
    manifest = manifest[manifest["graph_kind"] == "static"].copy()
    if len(manifest) != 18:
        raise SystemExit(f"Expected 18 static ONNX models, found {len(manifest)}")
    ...
    for number, item in enumerate(manifest.itertuples(index=False), start=1):
        ...
        print(f"[ST] {number:02d}/18 {model_id}", flush=True)
    ...
    print("ST Edge AI 18/18 PASS", flush=True)

...
REPO = DROP.parent
PACKAGE_ROOTS = {
    "NMC": REPO / "inference_pkg_nmc",
    "LFP": REPO / "inference_pkg_lfp",
}
RECORD_ROOTS = {
    "NMC": Path("${SOURCE_WORKSPACE}/DL/CEMA_MLP_OCVSTART_FULLGRID/")
    / "nmc_ocvstart_25c_us06_soc0_dstfudsmean_lopo_clean",
    "LFP": Path("${SOURCE_WORKSPACE}/LFP_REMOTE_EXACT_COPY_20260713/DL/CEMA_LFP/")
    / "lfp_confirmatory_minipanel/prepared_data_ocv_discharge_soc",
}
...
def main() -> None:
    ...
    for index, row in enumerate(manifest.itertuples(index=False), start=1):
        if not started:
            ...
            if not started:
                continue
        print(f"[raw VIT] {index:02d}/18 {row.model_id}: build", flush=True)
        ...
        package_path = (
            PACKAGE_ROOTS[row.chemistry] / "T6" / row.fold / str(row.seed)
        )
        ...
        stack_highwater = 0
        print(
            f"[raw VIT] {index:02d}/18 {row.model_id}: "
        ...
        print(
            f"[raw VIT] {index:02d}/18 {row.model_id}: {status}, "

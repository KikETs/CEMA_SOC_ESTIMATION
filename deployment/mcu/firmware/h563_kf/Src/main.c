...
static void CEMA_Send_Query(void)
{
  const uint32_t response[10] = {
      ...
      CEMA_KF_State_Dim(),
      CEMA_KF_Asset_Bytes(),
      CEMA_KF_Runtime_State_Bytes()};
  UART_Send(response, (uint16_t)sizeof(response));
}

static void CEMA_Send_Reset(const CEMA_KF_Reset *reset)
{
  uint32_t response[2];
  CEMA_KF_Reset_State(reset);
  g_model_status = CEMA_KF_Status();
  response[0] = CEMA_PROTOCOL_MAGIC;
  response[1] = g_model_status;
  UART_Send(response, (uint16_t)sizeof(response));
}

static void CEMA_Send_Sample(
    double voltage_v,
    double current_a,
    double temperature_c,
    double dt_s)
{
  CEMA_KF_Response response = {0};
  uint32_t primask;

  response.magic = CEMA_PROTOCOL_MAGIC;
  response.status = g_model_status;
  if (g_model_status != 0U)
  {
    UART_Send(&response, (uint16_t)sizeof(response));
    return;
  }
  ...
  DWT->CYCCNT = 0UL;
  response.output =
      CEMA_KF_Step(voltage_v, current_a, temperature_c, dt_s);
  ...
  response.sample_count = CEMA_KF_Sample_Count();
  response.state_dim = CEMA_KF_State_Dim();
  response.asset_bytes = CEMA_KF_Asset_Bytes();
  response.runtime_state_bytes = CEMA_KF_Runtime_State_Bytes();
  UART_Send(&response, (uint16_t)sizeof(response));
}
...
static void CEMA_Protocol_Loop(void)
{
  ...
  for (;;)
  {
    ...
    else if (command == CEMA_CMD_SAMPLE_DT)
    {
      if (HAL_UART_Receive(
              &huart3,
              (uint8_t *)&sample_dt,
